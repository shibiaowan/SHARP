...
sMetaC <-
    function(rerowColor,
        ...
        #     pind = which.max(t0)#the partition which has the most predicted clusters
        #     rind = setdiff(1:T, pind)
        #     #preprocessing
        sE1 = sE1[apply(sE1, 1, function(x)
            sd(x) != 0), ]#avoid NA when sd is 0 during scaling
        sE1 <- t(scale(t(sE1)))#vector scaling
        
        #     mE = matrix(0, nrow = length(R), ncol = dim(sE1)[2])
        #     for(i in 1:length(R)){
        #         G = sE1[rerowColor == R[i], , drop = FALSE]
        #         mE[i, ] = colMeans(G)*dim(G)[1]
        #     }
        #     mf = getwfperC(frowColor, E, p)#weighted features for each partitioned cluster
        cb = combn(nC, 2)#all possible combinations (n*(n-1)/2)
        #     t = p
        #     alls = apply(cb, 2, getpaircor, rerowColor = rerowColor, E1 = E1)#calculate the correlation/simialrity for all combinations (apply to the columns)
        ####parallel programming####
        alls = foreach(t = 1:dim(cb)[2], .combine = "c") %dopar% {
            getpaircor(cb[, t], rerowColor, sE1, R)
        }
        #     alls = foreach(t=1:dim(cb)[2], .combine = "c") %dopar%{exp(-sum((mf[cb[1, t]] - mf[cb[2, t]])^2)/t)}
        ...
        S0 = sparseMatrix(
            ...
            x = alls,
            dims = c(nC, nC)
        )#triangle part of the S
        S = S0 + t(S0) + diag(nC)
        
        #     w = matrix(0, nrow = length(R), ncol = 2)
        #     for(i in 1:length(R)){
        #         tind =setdiff(1:length(R), which(ff == ff[i]))#rest index
        #         w[i,1] = min(S[i, tind])
        #     }
        ...
        #     S = matrix(0, ncol = nC, nrow = nC)#initialization for a similarity matrix of all the clusters
        #     for(i in 1:nC){
        #         for(j in (i+1):nC){
        #             G1 = E1[rerowColor == allC[i], ]#find all the cells for cluster i
        #             G2 = E1[rerowColor == allC[j], ]#find all the cells for cluster j
        #             cor1 = cor(t(G1), t(G2))#correlation between these two clusters
        #             S[i, j] = (max.col(cor1) + max.col(t(cor1)))/(nrow(G1) + nrow(G2))#similarity between two clusters
        #             S[j, i] = S[i, j]
        #         }
        #         S[i, i] = 1
        #     }
        ...
        hres = get_opt_hclust(S,
                              hmethod,
                              N.cluster = finalN.cluster,
                              minN.cluster,
                              maxN.cluster,
                              sil.thre,
                              height.Ntimes)
        ...
        rerowColor[] <-
            vapply(rerowColor, function(x)
                tf[match(x, R)], numeric(1))#apply to every element;this is for matrices
        
        #   finalC = apply(rerowColor, 1, function(d) names(sort(table(d),decreasing=TRUE)[1])) #find the most repeated elements for each row
        
        #   f = rerowColor
        #   my = sE1
        #   silthre = 0
        # 	nthre = 0.05*length(f)
        # 	nf = character(length = length(f))
        # # 	if(hres$maxsil > silthre){
        # #             nf = as.character(f)
        # # 	}else{
        # 	uf = unique(f)
        # 	r = length(unique(f))
        # 	for(t in 1:r){
        #             tind = which(f==uf[t])
        #             if(length(tind)<= nthre){
        #                 nf[tind] = as.character(f[tind])
        #             }else{
        #
        #             tE = my[tind,]
        #
        #             d1=as.dist(1-cor(t(tE)))
        #             hres1 = gethclust(d1, tE)
        #
        #             rf = hres1$f
        #
        #             nf[tind] = paste(rf, "sub", t, sep = "")
        # 	    }
        #         }
        # #         }
        ...
        cat("The optimal number of clusters for combining partitions is: ",
            N.cluster,
            "\n")
...
getpaircor <- function(pairind, rerowColor, sE1, R) {
    ...
    aG1 = colMeans(G1)
    aG2 = colMeans(G2)
    #     corss = dist(rbind(aG1, aG2))
    #     corss = exp(-sum((aG1 - aG2)^2)/t)
    corss = cor(aG1, aG2)
    #     as1 = 1/(1 + exp(-aG1))
    #     as2 = 1/(1 + exp(-aG2))
    #     ass = sum(aG1*aG2)/sqrt(sum(aG1^2)*sum(aG2^2))
    #     corss = 1/(1 + exp(-ass))
    #     cor1 = foreach(i = 1:dim(G1)[1], .combine = 'cbind') %:%
    #                 foreach(j = 1:dim(G2)[1], .combine = 'c') %do%{
    # #                     sum(G1[i,]*G2[j,])/sqrt(sum(G1[i,]^2)*sum(G2[j,]^2))
    #                     exp(-sum((G1[i,] - G2[j,])^2)/t)
    #                 }
    #
    #     m = dim(cor1)
    #     if(m[1] <= m[2]){#use the smaller-size group to measure the correlation/similarity
    #         corss = sum(do.call(pmax, data.frame(cor1)))/nrow(G1)#similarity between two clusters; summing all the max correlation/similarity
    #     }else{
    #         corss = sum(do.call(pmax, data.frame(t(cor1))))/nrow(G2)
    #     }
    
    #     c1 = as.vector(cor1)
    #     c2 = c1[order(-c1)]#sorting the number in descending order
    #     corss = sum(c2[1:ceiling(length(c2)/2)])/sum(c2)
    #     c1 = do.call(pmax, data.frame(cor1))
    #     c2 = do.call(pmax, data.frame(t(cor1)))
    #     i0 = intersect(c1, c2)
    #     u0 = union(c1, c2)
    #     corss = sum(i0)/sum(u0)
    #     corss = (sum(do.call(pmax, data.frame(cor1))) + sum(do.call(pmax, data.frame(t(cor1)))))/(nrow(G1) + nrow(G2))#similarity between two clusters; summing all the max correlation/similarity
    #     corss = (sum(do.call(pmax, data.frame(cor1)))/nrow(G1) + sum(do.call(pmax, data.frame(t(cor1))))/nrow(G2))/2
    #     corss = max(max(cor1))
    #     print(corss)
    #     corss = mean(mean(cor1))
    return(corss)
}
