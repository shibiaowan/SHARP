...
get_opt_hclust<- function(mat, hmethod, N.cluster, minN.cluster, maxN.cluster, sil.thre, height.Ntimes){
	...
	#just use simple criteria to determine whether they are feature vectors or similarity matrix, and then we use different ways to measure the distance
	if(isSymmetric(mat)){#symmmetric matrix
            d=as.dist(1-mat)
            flag = 1
        }else{
...
# 	oind = which.max(msil)#the index corresponding to the max sil index
	...
	if(max(msil)<= sil.thre){
	  ...
	  if(oind ==1){#if the maximum CH index with the minimum number of clusters, it's likely that the CH index is not reliable either
