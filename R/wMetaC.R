...
wMetaC <-
    function(nC,
        ...
        #   vtmp = prcomp(AA, rank. = 2)#select the top-2 PCs
        #   vAA = vtmp$x#the 2-dim data
        
        #   AA = sum(apply(nC, 2, getA))/C
        indA = which(AA != 0, arr.ind = T)#find non-zero indices of AA
        nd = vapply(AA[indA], function(x)
            x * (1 - x), numeric(1))
        #   ####parallel programming####
        #   nd = foreach(t=1:dim(indA)[1], .combine = "c") %dopar%{AA[indA[t,]]*(1-AA[indA[t,]])}#calculate the weight s for all combinations
        ...
        newnC <-
            matrix(x, nrow = N, byrow = FALSE)#reshape a vector to a matrix; by column
        ...
        alls = apply(cb,
                     ...
                     R = R,
                     x = x,
                     w1 = w1)#calculate the weight s for all combinations
        
        #   ####parallel programming####
        #   alls = foreach(t=1:dim(cb)[2], .combine = "c") %dopar%{getss(cb[,t], R, x, w1)}#calculate the weight s for all combinations
        ...
        hres = get_opt_hclust(S,
                              hmethod,
                              N.cluster = enN.cluster,
                              minN.cluster,
                              maxN.cluster,
                              sil.thre,
                              height.Ntimes)#solely using the silhouette index as the criteria
        ...
        newnC[] <-
            vapply(newnC, function(x)
                tf[match(x, R)], numeric(1))#apply to every element; reorganizing the clusters for different results
        
        finalC = apply(newnC, 1, function(d)
            names(sort(table(d), decreasing = TRUE)[1])) #find the most repeated elements for each row
        ...
        cat("The optimal number of clusters for ensemble clustering is: ",
            N.cluster,
            "\n")
        
        
        #   #select representative cells/data
        #   ncp = 100#number of cells per cluster selected
        #   seind = numeric()#empty/initialization
        #   s_newnC = apply(newnC, 1, sort(table(d),decreasing=TRUE)[1]/length(d))#the score of the most-voting one
        #   for(j in 1:N.cluster){
        # 		nj = which(finalC == j)#number of cells/data in cluster j
        # 		tmp = s_newnC[nj]#selected scores
        # 		njo = order(-tmp)#order in descending order
        # 		if(length(njo) >= ncp){#if larger than threhold, we selected the first 100 data/cells
        #                     njj = nj[njo[1:ncp]]
        #                     seind = c(seind, njj)
        # 		}else{#else just select all
        #                     seind = c(seind, nj)
        # 		}
        #   }
        #   f_seind = finalC[seind]#the predicted clusters for the selected cells
...
getA <- function(rowColor) {
    ...
    #   ####parallel programming####
    #   tmp = foreach(t=1:length(L), .combine = "c") %dopar%{}#calculate the weight s for all combinations
...
getss <- function(pind, R, x, w1) {
    ...
    pairk = lapply(pind,
                   ...
                   R = R,
                   x = x,
                   N = length(w1))#run for two indices
