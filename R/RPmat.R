...
#'
#' @export
RPmat <- function(scdata, p, seedn){#for random projection; note scdata: m*n, m is the feature dimensions, n is the sample number; p is the reduced dimension
...
#   set.seed(123)#a flag to fix the random number
  ...
  if(seedn%%1==0){#integer
    set.seed(seedn); x0 = sample(c(sqrt(s),0, -sqrt(s)), size= m*p, replace=TRUE, prob=c(1/(2*s), 1 - 1/s, 1/(2*s)))
  }else{
