...
SHARP <-
    function(scExp,
        ...
        #timing
        start_time <-
            Sys.time()#we exclude the time for loading the input matrix
        ...
        if (!missing(exp.type)) {
            ...
            } else if (exp.type == "CPM" ||
                       exp.type == "TPM" || exp.type == "FPKM" || exp.type == "RPKM") {
                scExp = log10(scExp + 1)
            }
        }
        ...
        if (missing(base.ncells)) {
            #threshold of number of cells to determine whether SHARP_small or SHARP_large should be used
            base.ncells = 5000
        }
        ...
        cat("The dimension has been reduced from ",
            ngenes,
            " to ",
            reduced.ndim,
            "\n")
        ...
        t <-
            difftime(end_time, start_time, units = 'mins')#difference time in minutes
        print(t)
        ####################################
...
SHARP_small <-
    function(scExp,
        ...
        enE <-
            matrix(0, nrow = ncells, ncol = p)#already tranposed; reshuffled matrix
        for (j in 1:z) {
            #partition
            z1 = allrpinfo[[j]]
            enrp[, j] = z1$rowColor
...
SHARP_large <-
    function(scExp,
        ...
        enrp <-
            matrix('0', ncells, K)#the ensemble results after several random projection;namely several rowColor's
        ...
        if (T > 1) {
            folds <-
                cut(seq(1, T * ng), breaks = T, labels = FALSE)#note that the index may be larger than the number of cells
            ...
            cat("The number of cells in Folds ",
                names(table(folds)),
                "are: ",
                table(folds),
                "\n")
        ...
        {
            #nested looping; the first for different applications of random projection; the second for different partitions
            ...
            tmp = getrowColor(
                ...
                sil.thre,
                height.Ntimes
            )#hierarchical clustering; the result is a list containing both the predicted clusters and the maximum silhouette index
        ...
        frowColor = foreach(t = 1:T, .combine = "c") %dopar% {
            ...
            ftmp = wMetaC(
                ...
                sil.thre,
                height.Ntimes
            )#ensemble of several applications of RPs
